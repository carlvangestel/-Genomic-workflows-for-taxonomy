...
```
The code above loops over all bam files where the -b flag ensures the output remains in BAM format, -q 60 keeps only reads with a mapping quality of at least 60, -f 2 retains only properly paired reads and -F flag allows you to exclude alignments with certain SAM characteristics (see https://samtools.github.io/hts-specs/SAMv1.pdf for a detailed outline). Here, -F 2308 excludes unmapped reads, secondary alignments, and supplementary alignments. Next, the filtered output is directly piped into samtools sort, which sorts the alignments and writes them to a new bam file with the extansion '.clean.sorted.bam'. Finally, samtools index creates again an index file for each sorted BAM file. We're now ready to call SNPs based on our high-quality alignments.
