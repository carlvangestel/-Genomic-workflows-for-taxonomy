...
# SNP calling
bcftools mpileup --min-MQ 60 -a AD,DP,SP -Ou -f "$GENOME" Sample01_Loc1.clean.sorted.bam Sample02_Loc1.clean.sorted.bam Sample03_Loc2.clean.sorted.bam ... | bcftools call -f GQ,GP -m -Oz -o "$VCF_RAW"
```

The first part of the command, `bcftools mpileup`, generates the allelic count data for each individual. The `-f` flag specifies the reference genome, while `-a AD,DP,SP` adds annotations for allele depths (`AD`), total read depth (`DP`), and strand bias (`SP`) to the output. We also want to avoid SNPs to be called from reads that map ambiguously and filter them out by `–min-MQ 60` (minimum read mapping quality > 60), although this flag is redundant if alignments are already filtered during read mapping (MAPQ>60). The `-Ou` flag outputs an uncompressed VCF to standard output. The BAM files are listed at the end.
